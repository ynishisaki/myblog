...
（実践編）
1 WAV ファイルの準備
2 WAV ファイルのフォーマットについて説明
3 バイト列を int に変換
4 バイト列を ndarray に変換
5 WAV ファイルから、サンプリングレートと時系列データを取得
...
なお、値（エンコード前）は、**リトルエンディアン**表記である点に注意。

### fmt チャンク

| byte 位置 | size | 値（エンコード前） | 値（エンコード後） | 説明                                                                                   |
| --------- | ---- | ------------------ | ------------------ | -------------------------------------------------------------------------------------- |
...
| byte     | size  | 値（エンコード前） | 値（エンコード後） | 説明                                                                            |
| -------- | ----- | ------------------ | ------------------ | ------------------------------------------------------------------------------- |
...
サンプリングレートは 4 バイトの符号なし整数なので、上記２種類の方法で int 型に変換してみる。
...
一番明確な違いは、後者の`struct.unpack()`だと、値が tuple 型で返ってくることだろう。
複数の値を同時にエンコードすることが可能であるため、例えば、fmf チャンクの各値を全て変換する時に便利である。
また、[書式指定文字](https://docs.python.org/ja/3/library/struct.html?highlight=struct%20unpack#format-characters)（引数の`<h`部）を変えることで float 型への変換も可能である。
対して、前者の`int.to_bytes()`は、int 型への変換しか対応していないし、複数の値を同時変換することもできない（for ループとか使えばできるけど）。が、メソッド名も引数も簡潔で使いやすい。

今回はサンプリングレートを int に変換するだけなので、前者の`int.to_bytes()`で十分である。
もうすこし複雑な変換をするときは、後者の`struct.unpack()`が適していると思われる。

`int.to_bytes(bytes, byteorder, \*, signed=False)`
https://docs.python.org/ja/3/library/stdtypes.html?highlight=to_bytes#int.to_bytes
...
data = np.frombuffer(data_bytes, '<i2')  # "<" : リトルエンディアン, "i2" : integer 2byte
